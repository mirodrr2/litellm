...
async def user_api_key_auth_websocket(websocket: WebSocket) -> UserAPIKeyAuth:
    ...
    azure_api_key_header = headers.get(SpecialHeaders.azure_authorization.value)
    parent_otel_span: Optional[Span] = None
    start_time = datetime.now()
    try:
        route: str = get_request_route_websocket(websocket=websocket)
        ...
        if custom_litellm_key_header_name is not None:
            api_key = get_api_key_from_custom_header_websocket(
                websocket=websocket,
        ...
        ### USER-DEFINED AUTH FUNCTION ### #ToDo: implement custom auth for websocket
        ...
        if general_settings.get("enable_oauth2_proxy_auth", False) is True:
            return await handle_oauth2_proxy_request(websocket=websocket)
        ...
        if general_settings.get("enable_jwt_auth", False) is True:
            ...
            if is_jwt:
                ...
                # run through common checks ToDo: Implement common checks for webscoket
                # _ = common_checks(
        ...
        if is_mapped_pass_through_route:
            if websocket.headers.get("litellm_user_api_key") is not None:
                api_key = websocket.headers.get("litellm_user_api_key") or ""
...
def get_api_key_from_custom_header_websocket(
    websocket: WebSocket, custom_litellm_key_header_name: str
):
    # use this as the virtual key passed to litellm proxy
    custom_litellm_key_header_name = custom_litellm_key_header_name.lower()
    verbose_proxy_logger.debug(
    ...
    custom_api_key = websocket.headers.get(custom_litellm_key_header_name)
    if custom_api_key:
        api_key = _get_bearer_token(api_key=custom_api_key)
        verbose_proxy_logger.debug(
            "Found custom API key using header: {}, setting api_key={}".format(
